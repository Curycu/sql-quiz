# data touring
select 
  ub.user_id, u.status, u.signup, 
  ub.payment_id, p.price,
  (case 
    when sbb.title is not null then 'set'
    when b.series_title is not null then 'series'
    else 'book' end) as `book_type`,
  b.title, b.series_title, sbb.title as `set_book_title`,
  ub.service_type, ub.price as `book_price`
...
join ba.interview_book b on ub.book_id = b.id
left join (select distinct payment_id, set_book_id from ba.interview_set_book_sell) sb on sb.payment_id = p.id
left join ba.interview_book sbb on sb.set_book_id = sbb.id
;
